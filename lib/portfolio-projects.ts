...
export const SHOWCASE_PROJECTS: ProjectItem[] = [
  {
    ...
    detailParagraphs: [
      ...
      "공항, 항공사, 이용객 등 다양한 이해관계자들이 각자 다른 목적에 따라 정보를 탐색해야하는 복잡한 정보 구조와 비일관적인 UI를 검색 중심의 직관적이고 일관성을 높인 UI로 개선해 사용자가 원하는 정보를 빠르게 찾을 수 있게 재구성 하였습니다.",
      "또한 공공서비스의 정보에 신뢰감을 더해 줄 수 있는 컬러, 타이포그래피, 컴포넌트 등 UI 전반의 디자인 요소를 체계화하고 디자인 가이드를 수립했습니다.",
...
일회용컵 사용의 문제 인식은 가지고 있지만 실천이 번거로운 사용자를 위해 기능 위주 나열의 기존 화면을 재구성하여, 필요한 순간 망설임 없이 실행할 수 있는 핵심 기능 중심의 단순하고 직관적인 UI로 개선하였습니다.
...
실제 교육 현장 이미지를 활용해 서비스의 생동감을 전달하고, 교육·체험 정보와 예약 흐름을 직관적으로 설계해 사용 편의성을 높였습니다.`,
  },
  {
    ...
    detailTools: "Tool : Figma, XD, Photoshop, Illustrator",
    detailDescription: `모노플렉스의 예매/결제 및 관리 시스템 구축 디자인을 전담하였습니다.

모노플렉스는 프리미엄 영화관으로 브랜드의 고급스러움을 담고자 하였고,
판매자와 구매자가 동시에 보는 시스템임을 고려하여 명확하고 직관적인 UI로 시인성을 높이고 사용자 결제 동선을 최적화했습니다.`,
  },
  {
    id: 4,
    name: "헤르지온 앱 구축",
