...
[2개 질문의 역할 — 반드시 구분]
- **질문1**: 지금 막 이야기된 주제에 이어지는 질문. 지금까지와 같은 맥락에서 자연스럽고 깊어지지 않는 질문이어야 한다.
- **질문2**: 질문1과 **같은 세부 주제를 더 파고들지 않는다.** 반드시 **다른 카테고리(축)**로 넓혀서, 대화가 한 줄기로만 깊어지지 않게 한다.
  → 예: 앞에서 취미·러닝·운동 등이 나왔다면 질문2는 수상, 교육, 성장 방식, 협업 방식, 피그마/실무 프로세스, 포트폴리오, 팀 핏, 우리 서비스와의 연결, 커뮤니케이션 등 **전혀 다른 축** 중 하나를 고른다.
